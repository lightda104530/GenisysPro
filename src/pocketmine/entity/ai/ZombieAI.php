...
class ZombieAI{
	...
	public function __construct(AIHolder $AIHolder){
		$this->AI = $AI;
		if($this->AI->getServer()->aiConfig["zombie"] == 1){
			$this->AI->getServer()->getScheduler ()->scheduleRepeatingTask ( new CallbackTask ( [
				$this,
	...
	public function array_clear() {
		if (count($this->AI->zombie) != 0) {
			foreach ($this->AI->zombie as $eid=> $info) {
				foreach ($this->AI->getServer()->getLevels() as $level) {
					if (!($level->getEntity($eid) instanceof Entity)) {
						unset($this->AI->zombie[$eid]);
						//echo "清除 $eid \n";
					}
				}
			}
	...
	public function ZombieRandomWalkCalc() {
		$this->dif = $this->AI->getServer()->getDifficulty();
		//$this->getLogger()->info("僵尸数量：".count($this->plugin->zombie));
		foreach ($this->AI->getServer()->getLevels() as $level) {
			foreach ($level->getEntities() as $zo){
				if($zo instanceof Zombie){
					if ($this->AI->willMove($zo)) {
						if (!isset($this->AI->zombie[$zo->getId()])){
							$this->AI->zombie[$zo->getId()] = array(
								'ID' => $zo->getId(),
								'IsChasing' => false,
								'motionx' => 0,
								'motiony' => 0,
						...
						if ($zom['IsChasing'] === false) {  //自由行走模式
							...
							if ($zom['motionx'] == 0 and $zom['motionz'] == 0) {  //僵尸停止
								...
								//转向计算
								$yaw = $this->AI->getyaw($zom['motionx'], $zom['motionz']);
					...
					if (isset($this->AI->zombie[$zo->getId()])) {
						...
						if (!$zom['knockBack']) {
							...
							if ($zom['IsChasing'] !== false) {
								//echo ("是属于仇恨模式\n");
								$p = $this->AI->getServer()->getPlayer($zom['IsChasing']);
...
$xxx =0;$zzz=0;
 ...
 //严重加速bug	
			...
			//用来写僵尸宽度的
									...
									if ($zy === false or ($zy !== false and $this->AI->ifjump($zo->getLevel(), $pos0, true, true) == 'fall')) {  //前方不可前进
										//真正的自由落体 by boybook
										if ($this->AIHolder->ifjump($zo->getLevel(), $pos0, false) === false) { //原坐标依然是悬空
											...
											elseif ($xxx != 0 and $zzz != 0) {  //走向最近距离
												...
												} elseif ($this->AI->ifjump($zo->getLevel(), new Vector3($zo->getX(), $zo->getY() + 1, $zo->getZ() + $zzz), true) !== false) {
									...
									$pitch = $this->AI->getpitch($pos3,$ppos);
									$zom['yaw'] = $yaw;
									$zom['pitch'] = $pitch;
									if (!$zom['knockBack']) {
										$zom['x'] = $zo->getX();
